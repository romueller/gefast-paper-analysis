...
prepare_gefast_data <- function(data) {
  ...
  submethod_type[grepl("_[es]f1", data$cmd)] <- "-f1"
  submethod_type[grepl("_[es]2f", data$cmd)] <- "-2f"

  subsample <- substr(data$cmd, regexpr("sub_", data$cmd) + 4, regexpr("(.reduced)?.fasta", data$cmd) - 1)
...
prepare_usearch_data <- function(data) {
  ...
  threshold <- round(100 * (1 - as.numeric(substr(data$cmd, regexpr("-id ", data$cmd) + 4, regexpr("-uc ", data$cmd) - 2))))
  fastidious <- rep(F, nrow(data))
  fastidious_threshold <- rep(NA, nrow(data))
  submethod_type <- rep("-nf", nrow(data))
...
prepare_cdhit_data <- function(data) {
  ...
  data$cmd <- gsub("/bin/sh -c ", "", data$cmd)
  cmd <- as.character(substr(data$cmd, regexpr("/", data$cmd) + 1, regexpr(" ", data$cmd) - 1))
  threshold <- round(100 * (1 - as.numeric(substr(data$cmd, regexpr("-c ", data$cmd) + 3, regexpr("-d ", data$cmd) - 2))))
  fastidious <- rep(F, nrow(data))
  fastidious_threshold <- rep(NA, nrow(data))
  submethod_type <- rep("-nf", nrow(data))
...
prepare_dnaclust_data <- function(data) {
  ...
  data$cmd <- gsub("/bin/sh -c ", "", data$cmd)
  cmd <- as.character(substr(data$cmd, regexpr("/", data$cmd) + 1, regexpr(" ", data$cmd) - 1))
  threshold <- round(100 * (1 - as.numeric(substr(data$cmd, regexpr("-s ", data$cmd) + 3, regexpr("-i ", data$cmd) - 2))))
  fastidious <- rep(F, nrow(data))
  fastidious_threshold <- rep(NA, nrow(data))
  submethod_type <- rep("-nf", nrow(data))
...
prepare_sumaclust_data <- function(data) {
  ...
  data$cmd <- gsub("/bin/sh -c ", "", data$cmd)
  cmd <- as.character(substr(data$cmd, regexpr("/", data$cmd) + 1, regexpr(" ", data$cmd) - 1))
  threshold <- round(100 * (1 - as.numeric(substr(data$cmd, regexpr("-t ", data$cmd) + 3, regexpr("-O ", data$cmd) - 2))))
  fastidious <- rep(F, nrow(data))
  fastidious_threshold <- rep(NA, nrow(data))
  submethod_type <- rep("-nf", nrow(data))
...
# Creates visualisations of runtime and memory consumption 
# for different thresholds.
performance_facet_point_plots <- function(log_file, plot_file, with_legend = T, with_error_bars = F, dev = "pdf") {
  ...
  } else if (all(data$cmd %in% method_levels[4:9])) { # all except Swarm (v1 & v2) and GeFaST (edit distance)
    pal <- pal[4:9]
  ...
  facets <- ggplot(averaged_data, aes(x = threshold, y = ave_time, group = interaction(cmd, submethod_type), colour = cmd, shape = submethod_type)) + 
    geom_point(cex = 2.5, stroke = 1, position = position_dodge(width = 0.45)) + 
  ...
  if (with_error_bars) {
    facets <- facets + geom_errorbar(aes(ymin = ave_time - sd_time, ymax = ave_time + sd_time), width = 0.5, position = position_dodge(width = 0.45))
  }
  
  if (with_legend) {
    facets <- facets + theme(legend.position = c(0.3, 0.8), legend.direction = "vertical", legend.box = "horizontal")
  } else {
  ...
  facets <- ggplot(averaged_data, aes(x = threshold, y = ave_memory, group = interaction(cmd, submethod_type), colour = cmd, shape = submethod_type)) + 
    geom_point(cex = 2.5, stroke = 1, position = position_dodge(width = 0.45)) + 
    scale_shape_manual(values = c(1, 0, 5, 4, 7, 3, 12, 10)) + 
    scale_x_continuous(breaks = thresholds, minor_breaks = NULL, labels = paste0(thresholds, "\n", formatC(((100 - thresholds) / 100), digits = 2, format = "f"))) + 
    scale_colour_manual(values = pal) +
  ...
  if (with_error_bars) {
    facets <- facets + geom_errorbar(aes(ymin = ave_memory - sd_memory, ymax = ave_memory + sd_memory), width = 0.5, position = position_dodge(width = 0.45))
  }
  ...
  facets1 <- ggplot(averaged_data, aes(x = threshold, y = ave_time, group = interaction(cmd, submethod_type), colour = cmd, shape = submethod_type)) + 
    geom_point(cex = 2.5, stroke = 1, position = position_dodge(width = 0.45)) + 
    scale_shape_manual(values = c(1, 0, 5, 4, 7, 3, 12, 10)) +
    scale_colour_manual(values = pal) +
  ...
  if (with_error_bars) {
    facets1 <- facets1 + geom_errorbar(aes(ymin = ave_time - sd_time, ymax = ave_time + sd_time), width = 0.5, position = position_dodge(width = 0.45))
  }
  
  if (with_legend) {
    facets1 <- facets1 + theme(legend.position = c(0.3, 0.7), legend.direction = "vertical", legend.box = "horizontal")
  } else {
    facets1 <- facets1 + theme(legend.position = "none")
  }
  
  facets2 <- ggplot(averaged_data, aes(x = threshold, y = ave_memory, group = interaction(cmd, submethod_type), colour = cmd, shape = submethod_type)) + 
    geom_point(cex = 2.5, stroke = 1, position = position_dodge(width = 0.45)) + 
    scale_shape_manual(values = c(1, 0, 5, 4, 7, 3, 12, 10)) + 
    scale_x_continuous(breaks = thresholds, minor_breaks = NULL, labels = paste0(thresholds, "\n", formatC(((100 - thresholds) / 100), digits = 2, format = "f"))) + 
    scale_colour_manual(values = pal) +
  ...
  if (with_error_bars) {
    facets2 <- facets2 + geom_errorbar(aes(ymin = ave_memory - sd_memory, ymax = ave_memory + sd_memory), width = 0.5, position = position_dodge(width = 0.45))
  }
...
performance_facet_point_plots(log_file, paste0(plot_file, "_with_legend"), T, T)
performance_facet_point_plots(log_file, paste0(plot_file, "_without_legend"), F, T)
